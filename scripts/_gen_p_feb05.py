"""Generate _p2025sX_feb05.js PART modules from _q_rrcgdfeb05sX.json.
...
Usage: python _gen_p_feb05.py
"""
import json

HEADER = """// RRB Group D — 05 Feb 2026 CBT-1 Shift-{shift} (4 sections). iON CBT response-sheet — docx with
...
def gen(shift):
    with open(f"_q_rrcgdfeb05s{shift}.json", encoding="utf-8") as f:
        items = json.load(f)
    kept, dropped = [], []
    for it in items:
        o = it["orig"]
