"""Generate seed JS for IBPS PO Prelims paper.

Reads _questions_<slug>.json (from _parse_ibps_po_2016.py or _parse_ibps_po_official.py)
and produces a Node.js seed script that:
  1. Uploads images from _extracted_<slug>/ to Cloudinary at aajexam/pyq/ibps-po-prelims-<date-slug>/
  2. Creates IBPS-PO-PRE Exam + IBPS PO Prelims ExamPattern (100 Q × 3 sections, 60 min, 20-min sectional, 0.25 neg)
  3. Inserts PracticeTest with 100 questions and 5 options each

Usage: _gen_seed_ibps_po.py <slug> <date-slug> <pyq-year> <pyq-shift> <test-title> <output-js>
  e.g. _gen_seed_ibps_po.py ibps_po_2016 2016 2016 "" "IBPS PO Prelims - 2016" \
       scripts/seed-pyq-ibps-po-prelims-2016.js
"""
import sys, json, os
...
if len(qs) != 100:
    print(f"WARNING: expected 100, got {len(qs)} entries")
    # Pad to 100 with placeholders
    while len(qs) < 100:
        n = len(qs) + 1
        sec = 'English Language' if n <= 30 else 'Quantitative Aptitude' if n <= 65 else 'Reasoning Ability'
        qs.append({'n': n, 'section': sec, 'q': '', 'q_image': '', 'opts': ['','','','',''],
                   'opt_images': ['','','','',''], 'answer': 'A', 'sol': ''})
    qs = qs[:100]
...
KEY = [{'A': 1, 'B': 2, 'C': 3, 'D': 4, 'E': 5}.get(q['answer'], 1) for q in qs]

raw_items = []
for q in qs:
    sec = q.get('section') or 'English Language'
    opts = q.get('opts', ['','','','',''])
    ...
    item = (
        ...
        f"o: [{js_str(opts[0])}, {js_str(opts[1])}, {js_str(opts[2])}, {js_str(opts[3])}, {js_str(opts[4])}], "
        f"oi: [{js_str(opt_imgs[0])}, {js_str(opt_imgs[1])}, {js_str(opt_imgs[2])}, {js_str(opt_imgs[3])}, {js_str(opt_imgs[4])}], "
...
const CLOUDINARY_FOLDER = 'aajexam/pyq/ibps-po-prelims-{date_slug}';
const F = '{date_slug}';
...
async function buildQuestions() {{
  ...
  for (let i = 0; i < RAW.length; i++) {{
    ...
    questions.push({{
      ...
      explanation: r.e || '',
      section: r.s,
      tags: ['IBPS', 'PO', 'Prelims', 'PYQ', '{year_tag}'{shift_tag}],
      difficulty: 'medium'
    }});
  }}
  return questions;
