#!/usr/bin/env python3
"""Decode the image-based sections (Reasoning Part B, Numerical Part C) of a 2020-era
Delhi Police response-sheet docx. Each such question is laid out as four option figures,
EACH immediately followed by a marker image (green tick = correct, red cross = wrong),
then (usually) the question/stem figure with no marker. The tick position is the answer.

Builds a per-question composite PNG (stem figure on top + the four option figures labelled
1-4) and returns the answer index. Outputs _img2020_<slug>/sec<S>-<k>.png and a JSON list.

Usage: python scripts/_dpc2020_imgdecode.py <docx> <slug>
"""
...
# Linear token stream with section markers.
toks = []
sec = None
for p in paras:
    t = ptext(p)
...
# Group into questions: a FIG followed by a MARK is an option; collect 4 -> question.
# FIGs without a following MARK are stem figures (kept with the current question).
questions = []
cur_sec = None
i = 0
opts = []; stem = []
...
while i < len(toks):
    ...
    if k == 'FIG':
        ...
        if nxt[0] == 'MARK':
            ...
            if len(opts) == 4:
                # the immediately following FIG (no mark) is the stem
...
for q in questions:
    ...
    q['image'] = name
    built.append((q['section'], k, q['answerIndex'], len(q['stemFigs']), len(q['optionFigs'])))

from collections import Counter
print('image questions decoded:', Counter(s for s, *_ in built))
print('built (section,k,ans,#stem,#opt):', built)
json.dump([{'section': q['section'], 'answerIndex': q['answerIndex'], 'image': q['image']}
           for q in questions], open(f'_img2020_{SLUG}.json', 'w'), ensure_ascii=False, indent=1)
