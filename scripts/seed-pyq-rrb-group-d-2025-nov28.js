/**
 * Seed: RRB Group D (RRC Group D) PYQ — 2025 batch, 28 Nov 2025 CBT-1 (Shift-1/2/3)
 * Source: official iON CBT response-sheet papers — PDF ONLY (no docx supplied). Real text layer
 *   (stems + options extractable). Parsed by _parse_rrcgd25_pdf.py: text+bbox via pymupdf; the
 *   GREEN-TICK answer key is read by COLOUR from the rendered page (correct option's tick/text is
 *   green, wrong are red) since there are no docx tick-bullets to decode. Verified vs visible ticks
 *   + solvable math. Dropped: image-stem/image-option figure Qs, Tamil-garbled, fully-image Qs.
 * Reuses the Exam + ExamPattern records created by seed-pyq-rrb-group-d-2018.js (same CBT pattern).
 *
 * Run: node scripts/seed-pyq-rrb-group-d-2025-nov28.js
 */
...
import { PART as PS1 } from './_p2025s1_nov28.js';
import { PART as PS2 } from './_p2025s2_nov28.js';
import { PART as PS3 } from './_p2025s3_nov28.js';
...
async function seed() {
  ...
  const papers = [
    { title: 'RRB Group D - 28 November 2025 Shift-1', shift: 'Shift-1', parts: PS1 },
    { title: 'RRB Group D - 28 November 2025 Shift-2', shift: 'Shift-2', parts: PS2 },
    { title: 'RRB Group D - 28 November 2025 Shift-3', shift: 'Shift-3', parts: PS3 }
  ];
