"""Merge Aug-18 2022 transcriptions with deterministic green-tick keys -> _p2022sX_18.js

For each shift: read _tr_rrcgd18sX.json + _key_rrcgd18sX.txt, drop flagged/invalid Qs,
...
HEADER = """// RRB Group D — 18 Aug 2022 S{shift} ({sec_note}). iON CBT response-sheet source.
// Answer = GREEN-TICK option (decoded from tick-bullets, authoritative). Kept {kept}/100.
// Dropped: figure/graph/table/diagram, garbled/inconsistent, Bengali-only, non-linearizable.
...
def gen(shift):
    with open(f"_tr_rrcgd18s{shift}.json", encoding="utf-8") as f:
        items = json.load(f)
    key = {}
    with open(f"_key_rrcgd18s{shift}.txt") as f:
        for line in f:
    ...
    secs = sorted({k['s'] for k in kept})
    sec_note = '/'.join(secs)
    lines = [HEADER.format(shift=shift, sec_note='mixed sections', kept=len(kept))]
    for k in kept:
        lines.append(
            "  { orig:%d, s:%s, a:%d, q:%s, o:%s }," % (
                k['orig'], k['s'], k['a'], jstr(k['q']), jstr(k['o'])))
    lines.append("];\n")
    out = f"_p2022s{shift}_18.js"
