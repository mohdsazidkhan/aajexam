"""Generate _p2025sX_dec16.js PART modules from _q_rrcgddec16sX.json.
...
Usage: python _gen_p_dec16.py
"""
import json

HEADER = """// RRB Group D — 16 Dec 2025 CBT-1 Shift-{shift} (4 sections). iON CBT response-sheet — docx with
...
def gen(shift):
    with open(f"_q_rrcgddec16s{shift}.json", encoding="utf-8") as f:
        items = json.load(f)
    kept, dropped = [], []
    for it in items:
        o = it["orig"]
