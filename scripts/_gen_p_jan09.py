"""Generate _p2025sX_jan09.js PART modules from _q_rrcgdjan09sX.json.
...
Usage: python _gen_p_jan09.py
"""
import json

HEADER = """// RRB Group D — 9 Jan 2026 CBT-1 Shift-{shift} (4 sections). iON CBT response-sheet — docx with
...
def gen(shift):
    with open(f"_q_rrcgdjan09s{shift}.json", encoding="utf-8") as f:
        items = json.load(f)
    kept, dropped = [], []
    for it in items:
        o = it["orig"]
