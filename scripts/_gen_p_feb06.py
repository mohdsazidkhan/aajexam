"""Generate _p2025sX_feb06.js PART modules from _q_rrcgdfeb06sX.json.
...
Usage: python _gen_p_feb06.py
"""
import json

HEADER = """// RRB Group D — 06 Feb 2026 CBT-1 Shift-{shift} (4 sections). iON CBT response-sheet — docx with
...
def gen(shift):
    with open(f"_q_rrcgdfeb06s{shift}.json", encoding="utf-8") as f:
        items = json.load(f)
    kept, dropped = [], []
    for it in items:
        o = it["orig"]
