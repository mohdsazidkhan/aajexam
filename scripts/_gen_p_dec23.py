"""Generate _p2025sX_dec23.js PART modules from _q_rrcgddec23sX.json.
...
Usage: python _gen_p_dec23.py
"""
import json

HEADER = """// RRB Group D — 23 Dec 2025 CBT-1 Shift-{shift} (4 sections). iON CBT response-sheet — docx with
...
def gen(shift):
    with open(f"_q_rrcgddec23s{shift}.json", encoding="utf-8") as f:
        items = json.load(f)
    kept, dropped = [], []
    for it in items:
        o = it["orig"]
