"""Parse IBPS PO Prelims papers in Official IBPS format (2018+).

Source format:
  - Paper docx stream has `Que. N\\t<question text>` then 5 option paragraphs.
  - Last 5 non-empty paragraphs before next `Que.` are the 5 options.
  - Options may have `N.\\t` prefix or be bare text.
  - Answer key PDF has `Que. N Correct Option - X` (X in 1..5) OR table format
    `1 2 26 2 51 1 76 1` (Q-number Q-answer columns).
  - Section detection: by question-number range (configurable per paper).

Output: _questions_<slug>.json with {n, section, q, q_image, opts[5], opt_images[5], answer, sol}

Usage: _parse_ibps_po_official.py <stream_file> <answer_key_pdf> <slug> [section_layout]
  section_layout: comma-separated like "ENG:1-30,REA:31-65,QA:66-100"
  default: 'ENG:1-30,QA:31-65,REA:66-100'
"""
...
# AK PDF can be empty string (or '-') for papers with no separate AK file
# (e.g., memory-based papers where answers are inline as `Correct Option - N`).
ak_pdf = ak_pdf if ak_pdf and ak_pdf != '-' else ''
layout_arg = sys.argv[4] if len(sys.argv) > 4 else 'ENG:1-30,QA:31-65,REA:66-100'

SECTION_NAMES = {'ENG': 'English Language', 'REA': 'Reasoning Ability', 'QA': 'Quantitative Aptitude'}
def parse_layout(layout):
    """Parse 'ENG:1-30,QA:31-65,REA:66-100' -> dict mapping q-num to section name."""
    out = {}
...
def parse_block(raw, qnum):
    ...
    # Strip "Correct Option - N" lines (they're the answer, not an option)
    paras = [p for p in paras if not CORRECT_OPT_RE.match(p)]
    non_empty_idx = [i for i, p in enumerate(paras) if p]
    # Need at least 5 non-empty paragraphs (1 question + 4 options minimum).
    # Some 2019 paper questions ship with only 4 options instead of 5 — degrade gracefully.
    if len(non_empty_idx) < 5:
        return raw.strip(), ['', '', '', '', ''], '', ['', '', '', '', '']
...
if ak_pdf:
    ...
    if len(ans_map) < 100:
        for n, a in PAT_B.findall(ak_text):
            n_i = int(n)
            if 1 <= n_i <= 100 and n_i not in ans_map:
                ans_map[n_i] = int(a)
...
for n in range(1, 101):
    if n in parsed_by_n:
        ...
        q_text, opts, q_imgs_list, opt_imgs = '', ['', '', '', '', ''], [], ['', '', '', '', '']
    ...
    while len(opt_imgs) < 5: opt_imgs.append('')
    questions.append({
        'n': n,
