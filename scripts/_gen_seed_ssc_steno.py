"""Generate seed JS for SSC Stenographer Grade C & D Tier-I CBT paper.

Reads _questions_<slug>.json (from _parse_ssc_steno.py) and produces a Node.js
seed script that:
  1. Uploads images from _extracted_<slug>/ to Cloudinary at aajexam/pyq/ssc-steno-<date-slug>/
  2. Creates SSC-STENO Exam + 'SSC Stenographer Tier-I CBT' ExamPattern
     (200 Q × 4 options, 3 sections (50/50/100), 120 min, 0.25 negative)
  3. Inserts PracticeTest with 200 questions

Usage: _gen_seed_ssc_steno.py <slug> <date-slug> <pyq-year> <pyq-shift> <test-title> <output-js>
  e.g. _gen_seed_ssc_steno.py ssc_steno_11sep2017_s1 11sep2017-s1 2017 "11 September 2017 Shift-1" \\
       "SSC Stenographer Tier-I - 11 September 2017 Shift-1" scripts/seed-pyq-ssc-steno-11sep2017-s1.js
"""
import sys, json, os
...
if len(qs) != NUM_QS:
    ...
    while len(qs) < NUM_QS:
        n = len(qs) + 1
        sec = 'General Intelligence and Reasoning' if n <= 50 else 'General Awareness' if n <= 100 else 'English Language'
        qs.append({'n': n, 'section': sec, 'q': '', 'q_image': '',
...
JS = f'''/**
 * Seed: {test_title}
 * SSC Stenographer Grade C & D Tier-I CBT.
 * 200 Q × 1 mark, 3 sections: GI&R (50) / GA (50) / English (100), 120 min, 0.25 negative.
...
const CLOUDINARY_FOLDER = 'aajexam/pyq/ssc-steno-{date_slug}';
const F = '{date_slug}';
...
if (RAW.length !== {NUM_QS}) {{ console.error(`Expected {NUM_QS}, got ${{RAW.length}}`); process.exit(1); }}
if (KEY.length !== {NUM_QS}) {{ console.error(`KEY length ${{KEY.length}}`); process.exit(1); }}
...
async function buildQuestions() {{
  ...
  for (let i = 0; i < RAW.length; i++) {{
    ...
    questions.push({{
      ...
      explanation: r.e || '',
      section: r.s,
      tags: ['SSC', 'Stenographer', 'Tier-I', 'PYQ', '{pyq_year}'],
      difficulty: 'medium'
    }});
  }}
  return questions;
...
async function seed() {{
  ...
  let exam = await Exam.findOne({{ code: 'SSC-STENO' }});
  if (!exam) {{
    exam = await Exam.create({{
      category: category._id,
      name: 'SSC Stenographer Grade C & D',
      code: 'SSC-STENO',
      description: 'Staff Selection Commission - Stenographer Grade C and D Examination',
      isActive: true
    }});
    console.log('Created Exam: SSC-STENO');
  }}

  const PATTERN_TITLE = 'SSC Stenographer Tier-I CBT';
  ...
  if (!pattern) {{
    pattern = await ExamPattern.create({{
      ...
      sections: [
        ...
        {{ name: ENG, totalQuestions: 100, marksPerQuestion: 1, negativePerQuestion: 0.25 }}
      ]
    }});
    console.log('Created ExamPattern: SSC Stenographer Tier-I CBT');
  }}
  ...
  const test = await PracticeTest.create({{
    ...
    pyqExamName: 'SSC Stenographer Tier-I', questions
  }});
