        ...
        <div className="py-0 lg:py-6 space-y-2 lg:space-y-4">
          ...
          {mentor.examsCleared?.length > 0 && (
            <Card className="space-y-2">
              <h2 className="text-sm font-black text-slate-900 dark:text-white flex items-center gap-1.5"><Award className="w-4 h-4 text-primary-600" /> Exams Cleared</h2>
            ...
            <Card className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {mentor.specialization.map((s, i) => (
                  <span key={i} className="px-2.5 py-1 bg-primary-50 dark:bg-primary-900/30 text-primary-600 rounded-full text-[10px] font-bold">{s}</span>
                ))}
              </div>
            </Card>
          )}
              ...
              <ul className="space-y-1.5">
                {mentor.tips.map((t, i) => (
                  <li key={i} className="text-xs text-slate-600 dark:text-slate-300 flex gap-2">
                    <span className="text-primary-600 font-black">•</span> {t}
                  </li>
                ))}
              </ul>
            </Card>
          ...
          {mentor.booksRecommended?.length > 0 && (
            <Card className="space-y-2">
              <h2 className="text-sm font-black text-slate-900 dark:text-white flex items-center gap-1.5"><BookOpen className="w-4 h-4 text-primary-600" /> Books Recommended</h2>
              <ul className="space-y-1.5">
                {mentor.booksRecommended.map((b, i) => (
                  <li key={i} className="text-xs text-slate-600 dark:text-slate-300 flex gap-2">
                    <span className="text-primary-600 font-black">•</span> {b}
                  </li>
                ))}
              </ul>
            </Card>
