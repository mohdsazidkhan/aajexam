...
const AdminDailyChallenge = () => {
  ...
  const columns = [
    {
      key: 'date', header: 'Date', render: (_, c) => (
        <span className="text-slate-500">{new Date(c.date).toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}</span>
      )
    },
    {
      key: 'title', header: 'Title', render: (_, c) => (
        <span className="font-bold text-slate-900 dark:text-white">{c.title}</span>
      )
    },
    {
      key: 'questions', header: 'Questions', render: (_, c) => (
        <span className="text-slate-500">{c.questions?.length || 0}</span>
      )
    },
    {
      key: 'totalAttempts', header: 'Attempts', render: (_, c) => (
        <span className="text-slate-500">{c.totalAttempts}</span>
      )
    },
    {
      key: 'avgScore', header: 'Avg Score', render: (_, c) => (
        <span className="text-slate-500">{c.avgScore}</span>
      )
    },
    {
      key: 'status', header: 'Status', render: (_, c) => (
        <div className="flex items-center gap-2">
          <span className={`px-2 py-0.5 rounded text-[9px] font-black ${c.status === 'published' ? 'bg-primary-50 text-primary-600' : 'bg-slate-100 text-slate-500'}`}>{c.status}</span>
          ...
          {bulkProgress && (
            <Card className="shrink-0 mb-4">
              ...
              <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 mb-3 overflow-hidden">
                <div className="bg-primary-600 h-2 rounded-full transition-all duration-300" style={{ width: `${(bulkProgress.current / bulkProgress.total) * 100}%` }}></div>
              </div>
              <div className="flex gap-4 text-[10px] font-bold">
                <span className="text-slate-500">Total: {bulkProgress.total}</span>
                <span className="text-primary-600">Success: {bulkProgress.success}</span>
