...
import { Target, Plus, Zap, Trash2, Calendar, Table as TableIcon, List, LayoutGrid } from 'lucide-react';
    ...
    <AdminRoute>
      <div className="min-h-screen pb-24">
        ...
        <div className="py-4 lg:py-6">
          <div className='flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6'>
            <div className='flex flex-col'>
              <h1 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2 mb-2"><Target className="w-6 h-6 text-primary-700 shrink-0" /> Daily Challenges</h1>
            ...
            <div className="p-2 space-y-3">
              <div className="grid grid-cols-2 lg:flex lg:items-center gap-3">
                ...
                <button onClick={autoGenerateMonth} disabled={bulkGenerating} className="w-full lg:w-auto px-4 py-2 bg-primary-700 text-white rounded-lg lg:rounded-xl text-xs font-bold disabled:opacity-50 flex items-center justify-center">
                ...
                <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg lg:rounded-xl border border-slate-200 dark:border-slate-700">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs font-bold text-slate-700 dark:text-slate-300">
                      Generating: <span className="text-primary-700">{bulkProgress.currentDate}</span>
                    </span>
                  ...
                  <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 mb-3 overflow-hidden">
                    <div className="bg-primary-700 h-2 rounded-full transition-all duration-300" style={{ width: `${(bulkProgress.current / bulkProgress.total) * 100}%` }}></div>
                  </div>
                  <div className="flex gap-4 text-[10px] font-bold">
                    <span className="text-slate-500">Total: {bulkProgress.total}</span>
                    <span className="text-primary-700">Success: {bulkProgress.success}</span>
                    <span className="text-black dark:text-white">Skipped: {bulkProgress.skipped}</span>
                    <span className="text-black dark:text-white">Failed: {bulkProgress.failed}</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-bold text-slate-500 dark:text-slate-400">{challenges.length} challenge{challenges.length !== 1 ? 's' : ''} on this page</p>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-white dark:bg-slate-900">
                <tr>
                  <th className="px-4 py-3 text-left font-bold text-slate-500 uppercase text-xs">Date</th>
                  ...
                  <th className="px-4 py-3 text-left font-bold text-slate-500 uppercase text-xs">Attempts</th>
                  <th className="px-4 py-3 text-left font-bold text-slate-500 uppercase text-xs">Avg Score</th>
                  <th className="px-4 py-3 text-left font-bold text-slate-500 uppercase text-xs">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {challenges.map((c, i) => (
                  <tr key={c._id || i} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    <td className="px-4 py-3 text-slate-500">{new Date(c.date).toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}</td>
                    <td className="px-4 py-3 font-bold text-slate-900 dark:text-white">{c.title}</td>
                    <td className="px-4 py-3 text-slate-500">{c.questions?.length || 0}</td>
                    <td className="px-4 py-3 text-slate-500">{c.totalAttempts}</td>
                    <td className="px-4 py-3 text-slate-500">{c.avgScore}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 rounded text-[9px] font-black ${c.status === 'published' ? 'bg-primary-50 text-primary-700' : 'bg-slate-100 text-slate-500'}`}>{c.status}</span>
                        {c.isAutoGenerated && <span className="text-[9px] text-black dark:text-white font-bold">Auto</span>}
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {challenges.map((c, i) => (
              <Card key={c._id || i} className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-slate-500 truncate">{new Date(c.date).toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}</span>
                </div>
