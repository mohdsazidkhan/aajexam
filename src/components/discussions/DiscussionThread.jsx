        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1.5 flex items-center gap-3 text-xs">
            <button
              ...
              className={`flex items-center gap-1 hover:text-primary-700 transition ${myVote === 'up' ? 'text-primary-700 font-bold' : 'text-slate-500'}`}
            >
              <ThumbsUp className="w-3.5 h-3.5" /> {upvotes}
            </button>
            <button
              onClick={() => handleVote('down')}
              className={`flex items-center gap-1 hover:text-black dark:hover:text-white transition ${myVote === 'down' ? 'text-black dark:text-white font-bold' : 'text-slate-500'}`}
            >
...
export default function DiscussionThread({ questionId, sourceType, sourceId, defaultOpen = false }) {
  ...
  return (
    <div className="mt-3 border-t border-slate-200 dark:border-slate-700 pt-2">
      <button
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-primary-700 transition"
      >
            ...
            <div className="flex gap-2 mb-2 text-[10px] font-bold">
              {['top', 'new'].map(s => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded uppercase tracking-wider ${sort === s ? 'bg-primary-700 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-500'}`}
                >
                  {s === 'top' ? 'Top' : 'Newest'}
                </button>
              ))}
          ...
          {page < totalPages && (
            <div className="flex justify-center pt-2">
              <button
                ...
                className="px-4 py-1.5 text-[11px] font-bold text-primary-700 hover:text-primary-700 disabled:opacity-50"
              >
