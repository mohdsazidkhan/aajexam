  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark bg-slate-950' : 'bg-slate-50'} transition-colors duration-500 font-nunito selection:bg-primary-500 selection:text-white`}>
      ...
      <main className={`transition-all duration-500 min-h-screen
        ${showAppNav ?
          (isSidebarOpen ? 'lg:ml-80 pt-16 lg:pt-28 pb-20 lg:pb-12' : 'ml-0 pt-16 lg:pt-24 px-0 pb-20 lg:pb-12') :
          (isQuestPage ? 'p-0 m-0 w-full overflow-hidden' : 'pt-16 lg:pt-20')
        }`}
      >
        <div className={`mx-auto transition-all duration-500
          ${showAppNav ? 'max-w-6xl px-0 md:px-6' : (isQuestPage ? 'max-w-full px-0' : 'max-w-7xl px-0 md:px-6')}
        }`}>
