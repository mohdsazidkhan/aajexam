                    ...
                    {summary && (
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                         {[
                           ...
                           <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-white/80 dark:bg-white/5 backdrop-blur-xl p-6 rounded-[2.5rem] border-2 border-slate-100 dark:border-white/10 shadow-lg group hover:border-primary-500/30 transition-all">
                    ...
                    <AnimatePresence mode="wait">
                      {loading ? (
                         ...
                         <div className="text-center py-32">
                            <div className="p-8 bg-rose-500/10 rounded-[3rem] mb-6 border-4 border-dashed border-rose-500/20 inline-block text-rose-500 text-6xl">!</div>
                         ...
                         <div className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl rounded-[4rem] border-4 border-dashed border-slate-200 dark:border-white/10 p-24 text-center">
                            <Trophy className="w-20 h-20 text-slate-300 mx-auto mb-8 opacity-20" />
                          ...
                          <motion.div key={viewMode} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                 ...
                                 <table className="w-full border-collapse">
                                    <thead>
                                       <tr className="bg-slate-50/50 dark:bg-white/5 border-b border-slate-100 dark:border-white/10 text-left">
                                          <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Date</th>
                                          <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Beneficiary</th>
                                          <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Reason / Description</th>
                                          <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Yield</th>
                                          <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Post-Balance</th>
                                       </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                                       {transactions.map((tx, i) => (
                                         <motion.tr key={tx._id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }} className="group hover:bg-primary-500/5 transition-all cursor-pointer">
                                            ...
                                            <td className="px-8 py-6 text-right font-black text-slate-900 dark:text-white tabular-nums text-sm">
                            ...
                            {viewMode === 'grid' && (
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                                 {transactions.map((tx, i) => (
                                   <motion.div key={tx._id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }} className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl rounded-[3rem] border-4 border-slate-100 dark:border-white/10 p-8 shadow-2xl group flex flex-col font-outfit hover:border-primary-500/20 transition-all">
                                      ...
                                      <div className="mt-8 pt-6 border-t border-slate-100 dark:border-white/10 flex justify-between items-end">
                                         <div>
                            ...
                            {viewMode === 'list' && (
                              <div className="space-y-6">
                                 {transactions.map((tx, i) => (
                                   <motion.div key={tx._id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl rounded-[2.5rem] border-4 border-slate-100 dark:border-white/10 p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-primary-500/30 transition-all font-outfit shadow-xl group">
                                      <div className="flex items-center gap-6">
                                         <div className="w-14 h-14 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[1.5rem] flex items-center justify-center font-black text-xl shadow-xl shrink-0 italic">{tx.user?.name?.[0] || 'U'}</div>
                                         <div>
