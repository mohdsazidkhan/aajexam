            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {filtered.map(sub => (
                <tr key={sub._id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                  ...
                  {showOrderColumn && <td className="px-4 py-3 text-slate-500">{sub.order}</td>}
              ...
              {filtered.length === 0 && <tr><td colSpan={showOrderColumn ? 4 : 3} className="px-4 py-8 text-center text-slate-400">No subjects found</td></tr>}
            </tbody>
          </table>
        </div>
      ) : viewMode === 'grid' ? (
          ...
          {filtered.map(sub => (
            <div key={sub._id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 flex flex-col gap-3">
              <div className="flex items-start justify-between gap-2">
                <div className="w-10 h-10 rounded-xl bg-primary-50 dark:bg-primary-900/20 text-primary-700 flex items-center justify-center shrink-0"><BookMarked className="w-5 h-5" /></div>
                <span className={`text-xs font-bold px-2 py-1 rounded-full ${sub.isActive ? 'bg-primary-100 text-primary-700' : 'bg-slate-100 dark:bg-slate-800 text-black dark:text-white'}`}>{sub.isActive ? 'Active' : 'Inactive'}</span>
              </div>
              <div>
              ...
              <div className="flex gap-2 mt-auto pt-2 border-t border-slate-100 dark:border-slate-700">
                <button onClick={() => openEdit(sub)} className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-bold text-primary-700 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg"><Edit3 className="w-3.5 h-3.5" /> Edit</button>
        ...
        <div className="space-y-2">
          {filtered.map(sub => (
            <div key={sub._id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-3 flex items-center gap-3">
              ...
              <span className={`text-xs font-bold px-2 py-1 rounded-full shrink-0 ${sub.isActive ? 'bg-primary-100 text-primary-700' : 'bg-slate-100 dark:bg-slate-800 text-black dark:text-white'}`}>{sub.isActive ? 'Active' : 'Inactive'}</span>
              <div className="flex gap-1 shrink-0">
                <button onClick={() => openEdit(sub)} className="p-1.5 text-primary-700 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg"><Edit3 className="w-4 h-4" /></button>
