  ...
  return (
    <div className="h-[calc(100vh-64px)] max-md:h-[calc(100vh-112px)] overflow-hidden flex flex-col font-outfit text-slate-900 dark:text-white">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-4 shrink-0">
        <h1 className="text-2xl font-black uppercase text-slate-900 dark:text-white flex items-center gap-2 shrink-0"><Layers className="w-6 h-6 text-black dark:text-white" /> Topics <span className="text-slate-400 dark:text-slate-500">({filtered.length})</span></h1>
        ...
        <div className="grid grid-cols-2 lg:flex lg:items-center gap-3 w-full lg:w-auto">
          ...
          <select value={filterSubject} onChange={e => setFilterSubject(e.target.value)} className="w-full lg:w-auto px-3 py-2 bg-slate-50 dark:bg-black border border-slate-300 dark:border-slate-700 rounded-lg lg:rounded-xl text-sm">
          ...
          <button onClick={openCreate} className="w-full lg:w-auto flex items-center justify-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg lg:rounded-xl font-bold text-sm hover:bg-primary-800 shrink-0"><Plus className="w-4 h-4"/> Add Topic</button>
        </div>
      </div>
          ...
          {filtered.map(t => (
            <div key={t._id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 flex flex-col gap-3">
              <div className="flex items-start justify-between gap-2">
                <div className="w-10 h-10 rounded-xl bg-primary-50 dark:bg-primary-900/20 text-primary-600 flex items-center justify-center shrink-0"><Layers className="w-5 h-5" /></div>
                <span className={`text-xs font-bold px-2 py-1 rounded-full ${t.isActive ? 'bg-primary-100 text-primary-600' : 'bg-slate-100 dark:bg-slate-800 text-black dark:text-white'}`}>{t.isActive ? 'Active' : 'Inactive'}</span>
              </div>
              <div>
