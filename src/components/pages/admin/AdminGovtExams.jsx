            ...
            <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
               <div className="flex-1 relative group w-full">
                 ...
                 <input
                   ...
                   className="w-full pl-14 pr-8 py-5 bg-white dark:bg-white/5 border-2 border-transparent focus:border-indigo-500/30 rounded-[2.5rem] text-[10px] font-black uppercase tracking-widest outline-none transition-all shadow-lg"
                 />
               </div>
 
               <div className="flex items-center gap-4 px-6 py-4 bg-white dark:bg-white/10 rounded-2xl shadow-inner border-2 border-slate-200/50 dark:border-white/5 min-w-[280px]">
          ...
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              <div className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl rounded-[3rem] border-4 border-dashed border-slate-200 dark:border-white/10 p-20 text-center">
                <Building2 className="w-16 h-16 text-slate-300 mx-auto mb-8 opacity-20" />
              ...
              <motion.div key={viewMode} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    ...
                    <table className="w-full border-collapse">
                       <thead>
                         <tr className="bg-slate-50/50 dark:bg-white/5 border-b border-slate-100 dark:border-white/10 text-left">
                           <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Registry ID</th>
                           <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Assessment Name</th>
                           <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Syllabus Category</th>
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                        {filteredExams.map((exam, idx) => (
                          <motion.tr key={exam._id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.03 }} className="group hover:bg-slate-50/50 dark:hover:bg-white/5 transition-all">
                            ...
                            <td className="px-8 py-6 font-black text-slate-900 dark:text-white uppercase italic tracking-tight">{exam.name}</td>
                            <td className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">{exam.category?.name || 'Uncategorized'}</td>
                            <td className="px-8 py-6">
                ...
                {viewMode === 'grid' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredExams.map((exam, idx) => (
                      <motion.div key={exam._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl rounded-[3rem] border-4 border-slate-100 dark:border-white/10 p-8 shadow-2xl relative font-outfit">
                ...
                {viewMode === 'list' && (
                  <div className="space-y-6">
                    {filteredExams.map((exam, idx) => (
                      <motion.div key={exam._id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl rounded-[2.5rem] border-4 border-slate-100 dark:border-white/10 p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-primary-500/30 transition-all font-outfit shadow-xl">
                        ...
                        <div className="flex items-center gap-3">
                           <Link href={`/admin/govt-exams/patterns?examId=${exam._id}`}>
                              <motion.button whileHover={{ scale: 1.05 }} className="px-8 py-3 bg-primary-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg">Manage Patterns</motion.button>
                           </Link>
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-2xl bg-white dark:bg-[#0A0F1E] rounded-[3rem] border-4 border-slate-100 dark:border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <form onSubmit={handleSubmit} className="p-8 overflow-y-auto custom-scrollbar">
                <div className="space-y-8">
                  <div className="space-y-4">
                    ...
                    <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} required className="w-full px-6 py-5 bg-slate-50 dark:bg-white/5 border-2 border-transparent focus:border-primary-500/30 rounded-2xl text-xs font-black uppercase tracking-widest outline-none appearance-none cursor-pointer">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Exam Name" required className="w-full px-6 py-5 bg-slate-50 dark:bg-white/5 border-2 border-transparent focus:border-primary-500/30 rounded-2xl text-xs font-black uppercase outline-none shadow-inner" />
                    </div>
                  </div>
                  <div className="space-y-4">
                    ...
                    <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Exam Details" rows="4" className="w-full px-8 py-6 bg-slate-50 dark:bg-white/5 border-2 border-transparent focus:border-primary-500/30 rounded-[2rem] text-xs font-black uppercase outline-none shadow-inner resize-none" />
                  </div>
                  <div className="space-y-4">
                    ...
                    <input type="text" value={formData.logo} onChange={(e) => setFormData({ ...formData, logo: e.target.value })} placeholder="https://..." className="w-full px-6 py-5 bg-slate-50 dark:bg-white/5 border-2 border-transparent focus:border-primary-500/30 rounded-2xl text-xs font-black outline-none shadow-inner" />
                  </div>
