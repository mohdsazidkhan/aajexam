      ...
      <div className="adminContent w-full mx-auto text-slate-900 dark:text-white font-outfit flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            <motion.div key={viewMode} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 min-h-0 flex flex-col">
              {viewMode === 'table' ? (
        ...
        {/* External Pagination (list view only — table view attaches its own below the table) */}
        {!loading && total > 0 && viewMode !== 'table' && (
          <div className="shrink-0 mt-4 lg:mt-12 flex justify-center">
            <Pagination currentPage={page} totalPages={pagination.totalPages || 1} onPageChange={setPage} totalItems={total} itemsPerPage={itemsPerPage} onItemsPerPageChange={handleLimitChange} />
          </div>
        )}
        </div>
