          ...
          {STEPS.map((s, i) => (
            <React.Fragment key={s.key}>
              <div className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap ${
                i < activeStep ? 'bg-primary-100 text-primary-600 dark:bg-primary-900/40 dark:text-primary-300'
                  : i === activeStep ? 'bg-primary-600 text-white'
          ...
          <div className="mb-4 flex flex-wrap items-start justify-between gap-2">
            <div>
              <h2 className="text-xl font-bold flex items-center gap-2">
                <PenSquare className="text-primary-600" /> Custom Email Template
              </h2>
            ...
            <div className="space-y-2 lg:space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex rounded-lg overflow-hidden border border-slate-300 dark:border-white/10 text-xs">
                    <button type="button" disabled={!canEditFields} onClick={() => setMode('simple')} className={`px-3 py-1 disabled:opacity-50 ${mode === 'simple' ? 'bg-primary-600 text-white' : 'bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300'}`}>Simple</button>
                    <button type="button" disabled={!canEditFields} onClick={() => setMode('html')} className={`px-3 py-1 disabled:opacity-50 ${mode === 'html' ? 'bg-primary-600 text-white' : 'bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300'}`}>HTML</button>
                  </div>
                </div>
                <textarea
                  rows={8}
              ...
              {isDraft && campaign && (
                <div className="border-t border-slate-100 dark:border-white/10 pt-4">
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                    Send Test Email
                    {campaign.testSentAt && <span className="ml-2 text-[10px] text-primary-600">✓ sent to {campaign.testSentTo}</span>}
                  </label>
              ...
              {isDraft && campaign && (
                <button onClick={publishNow} disabled={busyAction}
                  className="w-full py-3 px-4 rounded-lg flex items-center justify-center gap-2 font-medium bg-primary-600 hover:bg-primary-800 text-white shadow-sm disabled:opacity-50">
                  🚀 Publish Now
                </button>
              )}
              ...
              {isPublished && (
                <div className="space-y-3 border-t border-slate-100 dark:border-white/10 pt-4">
                  <button onClick={sendToAll} disabled={busyAction}
                    className="w-full py-3 px-4 rounded-lg flex items-center justify-center gap-2 font-medium bg-primary-600 hover:bg-primary-600 text-white shadow-sm disabled:opacity-50">
                  ...
                  {(isSendingPhase || isCompleted) && (
                    <>
                      <div className="w-full bg-slate-200 dark:bg-white/10 rounded-full h-3 overflow-hidden">
                        <div className="bg-primary-600 h-3 transition-all duration-500" style={{ width: `${pct}%` }} />
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-3 text-center text-sm">
                        <div className="p-2 rounded-lg bg-white dark:bg-white/5">
                          <div className="text-primary-600 font-bold">{campaign.sentCount}</div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        {isSendingPhase && isRunning && (
                          <button onClick={pauseDriver} className="flex-1 py-2 px-3 rounded-lg text-sm bg-primary-600 hover:bg-primary-800 text-white">Pause</button>
                        )}
                        {isSendingPhase && !isRunning && (
                          <button onClick={continueDriver} disabled={busyAction} className="flex-1 py-2 px-3 rounded-lg text-sm bg-primary-600 hover:bg-primary-600 text-white disabled:opacity-50">
                        ...
                        <>
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-xs text-slate-600 dark:text-slate-300">
                              Sent to <strong className="text-primary-600">{recipients.counts.sent}</strong>
