  ...
  return (
    <div className="h-[calc(100dvh-64px)] max-md:h-[calc(100dvh-112px)] overflow-hidden flex flex-col font-outfit text-slate-900 dark:text-white">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-4 shrink-0">
        <h1 className="text-2xl font-black uppercase text-slate-900 dark:text-white flex items-center gap-2 shrink-0"><HelpCircle className="w-6 h-6 text-black dark:text-white" /> Questions <span className="text-slate-400 dark:text-slate-500">({totalItems})</span></h1>

        <div className="grid grid-cols-2 lg:flex lg:items-center gap-2 lg:gap-3 w-full lg:w-auto">
          <div className="relative col-span-2 lg:w-56">
          ...
          <select value={filters.exam} onChange={e => { setFilters({ ...filters, exam: e.target.value, subject: "", topic: "" }); setPage(1); }} className="px-3 py-2 bg-slate-50 dark:bg-black border border-slate-300 dark:border-slate-700 rounded-lg lg:rounded-xl text-sm max-w-full lg:max-w-[160px] truncate">
            <option value="">All Exams</option>{exams.map(ex => <option key={ex._id} value={ex._id}>{ex.name}</option>)}
          </select>
          <select value={filters.subject} onChange={e => { setFilters({ ...filters, subject: e.target.value, topic: "" }); setPage(1); }} className="px-3 py-2 bg-slate-50 dark:bg-black border border-slate-300 dark:border-slate-700 rounded-lg lg:rounded-xl text-sm max-w-full lg:max-w-[160px] truncate">
            <option value="">All Subjects</option>{subjects.map(s => <option key={s._id} value={s._id}>{s.name}</option>)}
          </select>
          <select value={filters.topic} onChange={e => { setFilters({ ...filters, topic: e.target.value }); setPage(1); }} className="px-3 py-2 bg-slate-50 dark:bg-black border border-slate-300 dark:border-slate-700 rounded-lg lg:rounded-xl text-sm max-w-full lg:max-w-[160px] truncate">
          ...
          <select value={filters.difficulty} onChange={e => { setFilters({ ...filters, difficulty: e.target.value }); setPage(1); }} className="px-3 py-2 bg-slate-50 dark:bg-black border border-slate-300 dark:border-slate-700 rounded-lg lg:rounded-xl text-sm">
