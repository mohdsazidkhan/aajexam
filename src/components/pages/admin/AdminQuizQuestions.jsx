  ...
  return (
    <div className="space-y-6 pt-4 lg:pt-6">
      ...
      <div className="flex flex-wrap gap-2">
        <select value={filters.exam} onChange={e => { setFilters({ ...filters, exam: e.target.value, subject: "", topic: "" }); setPage(1); }} className="px-3 py-2 bg-slate-50 dark:bg-black border border-slate-300 dark:border-slate-700 rounded-lg lg:rounded-xl text-sm">
          <option value="">All Exams</option>{exams.map(ex => <option key={ex._id} value={ex._id}>{ex.name}</option>)}
        </select>
        <select value={filters.subject} onChange={e => { setFilters({ ...filters, subject: e.target.value, topic: "" }); setPage(1); }} className="px-3 py-2 bg-slate-50 dark:bg-black border border-slate-300 dark:border-slate-700 rounded-lg lg:rounded-xl text-sm">
          <option value="">All Subjects</option>{subjects.map(s => <option key={s._id} value={s._id}>{s.name}</option>)}
        </select>
        <select value={filters.topic} onChange={e => { setFilters({ ...filters, topic: e.target.value }); setPage(1); }} className="px-3 py-2 bg-slate-50 dark:bg-black border border-slate-300 dark:border-slate-700 rounded-lg lg:rounded-xl text-sm">
