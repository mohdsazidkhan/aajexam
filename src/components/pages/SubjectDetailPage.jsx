...
import { ArrowLeft, BookMarked, FileText, BrainCircuit, Play, Trophy, Clock } from 'lucide-react';
...
const SubjectDetailPage = () => {
  ...
  useEffect(() => {
    ...
    API.getSubjectDetail(id).then(res => {
      if (res.success) { setSubject(res.subject); setPracticeTests(res.practiceTests || []); setQuizzes(res.quizzes || []); }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-24">
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="bg-gradient-to-br from-indigo-500 to-violet-600 rounded-2xl p-6 text-white mb-5">
          ...
          <p className="text-sm opacity-80 mt-1">{subject.exam?.name || 'General'}</p>
        ...
        <div className="flex gap-2 mb-5 sticky top-16 z-20 backdrop-blur-xl py-3 -mx-4 px-4 border-b border-slate-100 dark:border-slate-800">
          {tabs.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-black uppercase text-xs border-b-4 ${activeTab === tab.key ? 'bg-indigo-500 text-white border-indigo-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700'}`}>
            ...
            {practiceTests.length === 0 ? <div className="py-16 text-center"><FileText className="w-12 h-12 text-slate-200 mx-auto mb-2" /><p className="text-sm text-slate-400">No practice tests</p></div> : practiceTests.map((test, idx) => {
              ...
              return (
                <motion.div key={test._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
                  className={`flex items-center gap-4 p-4 bg-white dark:bg-slate-800 rounded-xl border ${done ? 'border-indigo-200 dark:border-indigo-800' : 'border-slate-200 dark:border-slate-700'}`}>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${done ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-400'}`}>
            ...
            {quizzes.length === 0 ? <div className="py-16 text-center"><BrainCircuit className="w-12 h-12 text-slate-200 mx-auto mb-2" /><p className="text-sm text-slate-400">No quizzes</p></div> : quizzes.map((quiz, idx) => (
              <motion.div key={quiz._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400">{quiz.topic?.name || ''} · {quiz.duration} min · {quiz.totalMarks} marks</p>
                </div>
        ...
        {showTestModal && selectedTest && <TestStartModal isOpen={showTestModal} onClose={() => setShowTestModal(false)} onConfirm={() => { setShowTestModal(false); router.push(`/govt-exams/test/${selectedTest._id}/start`); }} test={selectedTest} pattern={selectedTest.examPattern} exam={subject.exam} />}
      </div>
    </div>
  );
};
