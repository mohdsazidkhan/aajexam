      ...
      <div className="space-y-6 lg:space-y-8 animate-fade-in mt-0 pb-4">
         ...
         <SubscriptionGuard message="Upgrade to PRO to unlock advanced analytics, detailed performance tracking, and comprehensive insights into your learning progress.">
            ...
            <section className="relative">
               <Card className="bg-primary-700 text-white border-none shadow-aajexam-primary p-5 lg:p-8 overflow-hidden relative rounded-[2rem] lg:rounded-[3rem]">
            ...
            <section className="flex flex-nowrap overflow-x-auto no-scrollbar gap-2 rounded-[2rem] max-w-full px-0 py-4 lg:p-8">
               {TABS.map(tab => (
                  <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                     className={`flex-shrink-0 whitespace-nowrap flex items-center gap-2 px-5 py-2.5 rounded-full font-black text-xs uppercase transition-all ${activeTab === tab.key ? 'bg-primary-700 text-white shadow-lg' : 'text-content-secondary hover:text-content-primary'}`}>
            ...
            <AnimatePresence mode="wait">
               <motion.div key={activeTab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.2 }} className="space-y-6">
                  ...
                  {activeTab === 'wallet' && (
                     <>
                        {/* Balance Card */}
                        <Card className="bg-primary-700 text-white border-none shadow-aajexam-primary p-2 lg:p-4 rounded-[2rem] lg:rounded-[3rem]">
