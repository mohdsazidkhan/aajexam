...
const SubjectListPage = () => {
  ...
  return (
    <div className="space-y-6 lg:space-y-10 animate-fade-in bg-transparent font-outfit pb-10">
      ...
      {/* ── Hero ── */}
      <section className="relative rounded-[2.5rem] p-8 lg:p-12 overflow-hidden shadow-2xl border-b-8 border-indigo-600/20 dark:border-indigo-900/30">
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-500 via-violet-600 to-indigo-500 dark:from-slate-900 dark:via-indigo-900/40 dark:to-slate-900" />
        <div className="relative z-10 flex flex-col items-center gap-4 text-center">
          ...
          <h1 className="text-2xl lg:text-5xl font-black uppercase leading-tight text-white tracking-tighter">Subjects Hub</h1>
        ...
        <BookMarked className="absolute -bottom-10 -right-10 w-80 h-80 text-white/10 rotate-12 pointer-events-none" />
      </section>
          ...
          {filtered.map((sub, idx) => (
            <motion.div key={sub._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }}>
              <Card hoverable onClick={() => router.push(`/subjects/${sub.slug}`)}
                className="group p-5 flex flex-col gap-4 border-border-primary hover:border-indigo-500 transition-all rounded-[1.5rem] bg-background-surface shadow-lg">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shrink-0 shadow-md">
                  ...
                  <ChevronRight className="w-5 h-5 text-slate-300 group-hover:text-indigo-500 group-hover:translate-x-1 transition-all shrink-0 mt-1" />
                </div>
          ...
          {filtered.length === 0 && !loading && (
            <div className="col-span-full py-20 text-center space-y-4">
              ...
              <button onClick={() => { setActiveFilter('all'); setSearch(''); }}
                className="px-6 py-2.5 bg-indigo-500 text-white rounded-full font-black text-xs uppercase">View All</button>
            </div>
          )}
        </div>
      </section>
