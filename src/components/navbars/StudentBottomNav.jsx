...
const StudentBottomNav = () => {
  ...
  return (
    <>
        ...
        {showCreate && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 z-[70] max-h-[85%] overflow-y-auto rounded-t-3xl bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 shadow-sm"
                ...
                <div className="space-y-2.5">
                  {REEL_TYPES.map((type) => (
                    <motion.button
                      ...
                      onClick={() => {
                        setShowCreate(false);
                        router.push(`/reels/create?type=${type.value}`);
                      }}
      ...
      {/* Bottom Nav */}
      <nav aria-label="Main navigation" className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl border-t-2 border-slate-200 dark:border-slate-800 flex items-end justify-around px-1 pb-[env(safe-area-inset-bottom)] z-[135]">
        {navItems.map((item) => {
          // Center Plus button
          if (item.name === 'CREATE') {
            return (
