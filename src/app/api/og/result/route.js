...
export async function GET(request) {
  ...
  const bandColor = pctNum >= 90 ? '#58cc02' : pctNum >= 75 ? '#cbd5e1' : pctNum >= 50 ? '#f8fafc' : '#94a3b8';
  ...
  return new ImageResponse(
    (
      ...
      >
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '8px', background: '#58cc02' }} />

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '40px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ fontSize: '36px', fontWeight: 900, color: '#58cc02', letterSpacing: '-1px' }}>AajExam</div>
        ...
        <div style={{ display: 'flex', gap: '20px', marginBottom: '40px' }}>
          {rank && total && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '28px 32px', background: 'rgba(88, 204, 2, 0.12)', border: '2px solid rgba(88, 204, 2, 0.4)', borderRadius: '28px' }}>
              <div style={{ fontSize: '14px', fontWeight: 900, color: '#58cc02', letterSpacing: '3px', marginBottom: '8px' }}>ALL INDIA RANK</div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px' }}>
                <div style={{ fontSize: '68px', fontWeight: 900, color: '#58cc02', lineHeight: 1, letterSpacing: '-2px' }}>#{rank}</div>
                <div style={{ fontSize: '22px', fontWeight: 700, color: '#cbd5e1' }}>of {Number(total).toLocaleString('en-IN')}</div>
              </div>
            </div>
          )}
          {pct && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '28px 32px', background: 'rgba(255, 255, 255, 0.12)', border: '2px solid rgba(255, 255, 255, 0.4)', borderRadius: '28px' }}>
              <div style={{ fontSize: '14px', fontWeight: 900, color: '#cbd5e1', letterSpacing: '3px', marginBottom: '8px' }}>PERCENTILE</div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px' }}>
                <div style={{ fontSize: '68px', fontWeight: 900, color: '#cbd5e1', lineHeight: 1, letterSpacing: '-2px' }}>{pctNum.toFixed(1)}</div>
        ...
        <div style={{ display: 'flex', gap: '20px', marginBottom: '30px' }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '20px 28px', background: 'rgba(255,255,255,0.04)', border: '1px solid #334155', borderRadius: '20px' }}>
            ...
            <div style={{ fontSize: '40px', fontWeight: 900, color: '#58cc02', lineHeight: 1 }}>{score}</div>
          </div>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '20px 28px', background: 'rgba(255,255,255,0.04)', border: '1px solid #334155', borderRadius: '20px' }}>
            <div style={{ fontSize: '12px', fontWeight: 900, color: '#64748b', letterSpacing: '3px', marginBottom: '6px' }}>ACCURACY</div>
            <div style={{ fontSize: '40px', fontWeight: 900, color: '#f8fafc', lineHeight: 1 }}>{parseFloat(accuracy).toFixed(0)}%</div>
          </div>
        </div>
        ...
        <div style={{ position: 'absolute', bottom: '30px', left: '80px', right: '80px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 800, color: '#58cc02', letterSpacing: '3px' }}>SSC · RRB · BANKING · STATE POLICE</div>
        </div>
      </div>
    ),
